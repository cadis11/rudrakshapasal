...
export default async function Page({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  const qr  = process.env.FONEPAY_QR_URL || "/qr/fonepay.png";
  const label = process.env.FONEPAY_ACCOUNT_LABEL || "Fonepay";

  return (
    <main className="mx-auto max-w-xl px-6 py-10">
      <h1 className="text-2xl font-bold mb-4">Pay with Fonepay</h1>
      ...
      <div className="rounded-xl border border-white/10 bg-black/20 p-4 grid place-items-center mb-4">
        <img src={qr} alt="Fonepay QR" className="max-h-80 w-auto object-contain" />
      </div>
      ...
      <form action="/api/payments/manual/confirm" method="POST" className="rounded-xl border border-white/10 bg-black/20 p-4">
        ...
        <input type="hidden" name="provider" value="FONEPAY" />
        <div className="grid gap-3">
          <input name="ref" placeholder="Transaction ID / Ref" className="rounded bg-neutral-900 border border-white/15 px-3 py-2" />
          <input name="payerPhone" placeholder="Your phone used in Fonepay" className="rounded bg-neutral-900 border border-white/15 px-3 py-2" />
