export async function GET(req: Request) {
  ...
  const patch = {
    mutations: [{
      patch: {
        ...
        set: { "payment.status": payStatus, "payment.provider": "FONEPAY", "updatedAt": now },
        insert: { after: "history[-1]", items: [{ ts: now, event: `fonepay_${payStatus}`, by: "fonepay", amt }] }
      }
    }]
  };
  ...
  return Response.redirect(`${site}/en/thanks/${oid}`, 302);
}